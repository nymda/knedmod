...
void sigscanItems() {
    ...
    std::cout << "remove attribute     : " << std::hex << glb::oSOAS << std::endl;
    ...
    std::cout << "setBody              : " << std::hex << glb::oSetBody << std::endl;
    ...
    std::cout << "update body weird    : " << std::hex << glb::tdUpdateFunc << std::endl;

    glb::tdFunRuiner = (funRuiner)mem::FindPattern((PBYTE)"\x48\x8B\xC4\x55\x41\x54\x41\x55\x41\x56\x41\x57\x48\x8D\xA8\x28", "xxxxxxxxxxxxxxxx", GetModuleHandle(NULL));
    std::cout << "fun ruiner           : " << std::hex << glb::tdFunRuiner << std::endl;
